...
public class CoursePageController {
    ...
    public String courses(@RequestParam(value = "keyword", required = false, defaultValue = "") String keyword,
        ...
        model.addAttribute("courses", keyword.isBlank() ? courseService.findPublished()
            : courseService.search(keyword).stream().filter(course -> Boolean.TRUE.equals(course.getStatus())).toList());
    ...
    public String courseDetail(@PathVariable("id") Integer id, Model model, HttpSession session) {
        Course course = courseService.findById(id);
        List<Video> videos = videoService.findByCourse(id);
        List<Article> articles = articleRepo.findByCourse_IdAndStatusTrueOrderByCreatedAtDesc(id);
    ...
    public String videos(@RequestParam(value = "keyword", required = false, defaultValue = "") String keyword,
        ...
        model.addAttribute("videos", keyword.isBlank() ? videoService.findPublished()
            : videoService.search(keyword).stream().filter(video -> Boolean.TRUE.equals(video.getStatus())).toList());
        model.addAttribute("keyword", keyword);
    ...
    public String videoDetail(@PathVariable("id") Integer id, Model model, HttpSession session) {
        ...
        List<Video> relatedVideos = video != null && video.getCourse() != null
                ? videoService.findByCourse(video.getCourse().getId())
